...
# Step 3b. Repeating on sub-set with time-to-event data

As per reviewer suggestions, I now repeat the 2SLS analysis for the diseases, for which we have date at diagnosis, focusing on the subset of individuals for whom MDD diagnosis preceded the disease.

```{r}

# Adapt function to limit dataset to individuals where MDD preceded trait
...
tsls <- function(trait) {
  ...
  x <- fread(paste0("subset_", trait, ".tsv")) %>% 
    left_join(pheno, by = c("eid" = "IID"))
  
  x <- x %>% filter(!is.na(.[[trait]]) & !is.na(mdd_prs_pc))
...
# Run for selected traits

traits <- c("anemia", "celiac", "sinusitis", "cushing", "accidents", "alcdeath", "cannabisabuse", "selfharm")

results_classic <- map(traits, tsls)
results_classic_df <- rbindlist(results_classic)

colnames(results_classic_df) <- c("beta", "se", "t", "p", "outcome", "df")

fwrite(results_classic_df, "2slsl_results_classic_subset.tsv")
